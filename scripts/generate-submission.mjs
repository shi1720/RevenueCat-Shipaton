...
// 4. Editable process diagram.
{
  ...
  notes(s, 'Product mechanics: suggestProjects filters by next-step duration and energy, excludes finished projects, and ranks blockers after ready work. Checkpoint history stores context and cumulative session minutes. The core browser and native emulator loop is verified. Current automated checks: 154 unit tests and 30 desktop/mobile browser scenarios. Final physical-device behavior remains a release check.');
}
...
// 7. Transparent release status and useful validation.
{
  ...
  text(s, 'Live web app + Android preview\n154 unit + 30 browser tests\nNative Test Store flows verified\nLive coordinated account deletion', 71, 299, 535, 222, 27, { lineSpacing: 1.25 });
  text(s, 'RELEASE GATES', 680, 245, 520, 35, 18, { bold: true, color: C.violet });
  text(s, 'Samsung corporate seller approval\nPhysical store purchase testing\nProduction signing + store approval\nCompiled iOS release binary', 680, 299, 535, 222, 27, { lineSpacing: 1.25 });
  ...
  notes(s, 'Status snapshot September 16, 2026. The public web app is live at https://unpause-studio.web.app . Public narrated Android emulator demo: https://www.youtube.com/watch?v=jXpOlvDShRY . Real hosted Firebase signup, reload persistence, sign-out, sign-in, account deletion, and rejection after deletion passed. Local projects survive account deletion. Firebase is primary, with Supabase a configurable fallback. Accounts do not synchronize projects. 154 unit tests and 30 browser scenarios pass. Android preview exists. iOS source and bundle configuration are prepared but no compiled iOS binary is claimed. Native physical-device auth persistence and password-reset email delivery still need acceptance evidence. RevenueCat project projb008cb09 has configured Galaxy, iOS, Google Play and Test Store mappings for studio/default/$rc_lifetime/unpause_studio_lifetime. Actual native Test Store cancellation, failure, success, restoration, and cold restart passed. These are simulated transactions, not production revenue. Supabase hosts the live Firebase/RevenueCat coordinated deletion service; all 15 live checks passed, including stale authentication, safe replay and rate limits, with four disposable identities cleaned. Samsung registration is complete, but Android app registration is blocked on Corporate Commercial Distribution Seller approval. Devpost entry submitted: https://devpost.com/software/unpause-k9p21y . No qualifying store release or customer traction is claimed. Galaxy publication still needs seller approval, physical store-device billing verification, production signing, and store approval. Official event rules: https://revenuecat-shipaton-2026.devpost.com/rules . Release gates documented in docs/release/launch-checklist.md.');
}
