...
At the owner’s explicit request, the source repository and all release assets are now public. A scan of all 237 historical Git blobs found no matching private-key bodies, service-account JSON, OpenAI secrets, RevenueCat secrets or GitHub tokens. Anonymous GitHub API and asset requests verified public access. Devpost’s public Try it out section now includes the web app, source, Android release, PDF, presentation and YouTube links. Samsung was rechecked after registration confirmation and still blocks Android registration pending corporate commercial seller approval; no store URL exists to add.
