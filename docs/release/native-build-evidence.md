...
## Current standalone Galaxy preview, subsequent September 16 build

The preceding APK-specific sections are historical. The current `artifacts/builds/unpause-galaxy-preview.apk` is published as [v1.0.0-preview.3](https://github.com/shi1720/Unpause-Preview/releases/tag/v1.0.0-preview.3), 37,325,300 bytes, SHA-256 `2d7db0e824a329bd8893a038b0bc9054cc82663c0488b9c93f436856014a4e2d`. Gradle completed `assembleRelease` successfully in 1m 4s with 690 tasks. It is the Galaxy package, version 1.0.0/code 1, ARM64, min API 24 and target API 36, with bundled Hermes JavaScript and the configured secure deletion endpoint.

The signature verifies with the same Android Debug test certificate, and 16 KiB APK zipalignment passes. This remains a sideload preview, not a production-signed store release. Its 41 input hashes are preserved separately in [current source snapshot](evidence/source-snapshot-galaxy-final.json); the earlier snapshot was not overwritten.

An in-place update on the existing API 36 ARM64 emulator succeeded. Actual cold launch showed the existing Sample studio and the exact Sunday tote next-step note intact. The account dialog opened. A bounded attempt to sign in with a newly created disposable account was stopped after ADB text injection dropped characters, paced entry stalled, and UIAutomator returned null roots. Native account sign-in or account deletion is therefore **not verified on this exact APK**. The disposable identity was removed through Firebase Admin and its private credential file deleted. Earlier native account successes remain attached only to their earlier APK hashes.

An offline restart was attempted, but the initial screenshot was still loading; the home screen rendered after networking was restored. This is inconclusive for fully offline startup on this APK. The final UI was the local home screen with no credential form visible. The latest process had zero ReactNativeJS/AndroidRuntime error lines. The separate sandbox package and Metro state were left intact.

See [current native result](evidence/galaxy-final-preview-result.json). Real backend deletion is independently covered by the [15 passing live API checks](evidence/firebase-deletion-live-result.json). No physical Samsung device, real store transaction, or public store publication is inferred from either suite.
